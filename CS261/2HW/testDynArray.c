...
int main(int argc, char* argv[]){
    ...
    printf("\nHere is our additional test cases\n");
    printf("We started with a 2 size capacity\n");

    int cap = capDynArr(test);
    int size = sizeDynArr(test);

	assertTrue(isEmptyDynArr(test), "Checking if empty");
    addDynArr(test, 3);
	assertTrue(!isEmptyDynArr(test), "Making sure it's not empty");
    addDynArr(test, 5);
    size = sizeDynArr(test);
    printf("Array's contents are [3, 5]\nThe capacity is %d\n", cap); 
    assertTrue(EQ(size, cap), "Checking if the size equals capacity");
    addDynArr(test, 7);
    cap = capDynArr(test);
    printf("The array's contents are [3, 5, 7]\nThe capacity is %d\n", cap);
    assertTrue(EQ(cap, 4), "Proving Capacity is 4");
    swapDynArr(test, 0, 2);
    printf("The array's contents are now [7, 5, 3]\n");
    assertTrue(EQ(getDynArr(test, 0), 7), "Checking if first element is 7");
    assertTrue(EQ(getDynArr(test, 1), 5), "Checking if second element is 5");
    assertTrue(EQ(getDynArr(test, 2), 3), "Checking if third element is 3");
    putDynArr(test, 1, 1);
    assertTrue(EQ(getDynArr(test, 1), 1), "Checking if stack is [7, 1, 3]");
    removeAtDynArr(test, 2);
    assertTrue(!containsDynArr(test, 3), "We got rid of 3 in array");
    printf("\n");

    pushDynArr(test, 4);
    assertTrue(EQ(topDynArr(test),4), "Checking if the top of the stack is 4");
    printf("Used topDynArr to verify the top of the stack was 4\n");
    popDynArr(test);
    assertTrue(EQ(topDynArr(test),1), "Checking that the top of the stack is 1");
    printf("\n");

    removeDynArr(test, 7);
    assertTrue(!containsDynArr(test, 7), "Checking if we removed 7 with removeDynArr");
