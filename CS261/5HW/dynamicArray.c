...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
    ...
    if(right < max)
    {
        int small = _smallerIndexHeap(heap, right, left, compare);
...
{
    int size = sizeDynArr(heap) - 1;
    putDynArr(heap,0, getDynArr(heap,size));
    removeAtDynArr(heap,size);
    _adjustHeap(heap, size, 0, compare);
}
...
void _buildHeap(DynArr *heap, comparator compare)
{
    ...
    {
        _adjustHeap(heap, size, i, compare);
    }
}
/*
