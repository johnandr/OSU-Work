...
int main (int argc, const char * argv[]) {
    ...
    while(getWord(fileptr) != NULL)
    {
        key = getWord(fileptr);
        if(containsKey(hashTable,key,myCompare, hash2) == 1)
        {
            void *value = atMap(hashTable,key,myCompare, hash2) + 1;
            insertMap(hashTable, key, value, myCompare, hash2);
        }else
        {
            insertMap(hashTable, key,(void *) 1, myCompare, hash2);
        }
    }
...
//	removeKey(hashTable, "the", myCompare, hash2);
	printf("\nHast Table after removing 'and', 'me' ,and 'the'\n------------------------------------------------");
	printMap(hashTable,keyPrint,valPrint);
	printf("\n\nKey Values\n----------\n");
